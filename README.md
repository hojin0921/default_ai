# Project Name

<!-- Phase 0 이후 AI가 프로젝트 개요로 채운다. 템플릿 사용법: 원본 guide.md / TEMPLATE.md -->

## Overview

<!-- TODO: 무엇을 하는 프로젝트인지 -->

## Setup

<!-- 실행 가능한 산출물이 생기면 AI가 실제 설치·실행 명령을 채운다. 추측 금지. 상세는 docs/development.md -->

## Development

<!-- TODO: 로컬 개발 흐름. 상세는 docs/development.md -->

## Testing

<!-- TODO: 대표 테스트 명령. 상세는 docs/testing.md -->

## Deployment

<!-- TODO: 배포 개요. 상세는 docs/deployment.md -->

<!--
템플릿: TEMPLATE.md | 사용법: guide.md (§2-2 시니어 역할) / docs/ai/agent-workflow.md
클론 후: install-hooks (§ guide.md §1) — macOS/Linux `./scripts/install-hooks.sh` · Windows `scripts\install-hooks.cmd`

A. 첫 메시지: 무엇을 만들지 (§3-0 안내) → 질문 → 전체 설계 합의 → docs → Plan(Phase) Draft → 채팅에서 승인 (+ gate approve-plan)
B. Phase마다 6단계 (Skill: delivery-phase + senior-*) → User Test Guide → 실행 가이드(켤 수 있으면) → 검수 → 다음 Phase. 마지막 Phase면 역할 기여(누가 무엇을)도.
Skills: .cursor/skills/ (kickoff / delivery-phase / phase-gate / senior-*) | Gate: 채팅 선택→gate.sh
-->
