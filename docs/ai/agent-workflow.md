...
| 결정 지점 번호 메뉴 제시 | 채팅에서 선택 (또는 동등 터미널 명령) |
...
전진 채널: **채팅 번호 선택 → AI가 `./scripts/gate.sh` 대행**, 또는 사람이 같은 명령을 터미널에서 실행.  
...
메뉴 `2`를 고를 때는 `2`와 수정 프롬프트를 **한 메시지에** 같이 쓰거나, `2` 다음에 이어서 쓰면 된다. 수정 후 AI는 메뉴를 다시 낸다.
